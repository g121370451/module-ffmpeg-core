// test/test_player.ts
import { NativeLoader } from '../utils/native_loader';
import type {IFFmpegModule, MediaManager} from '../lib/index';
...
describe('插件加载测试', () => {
  // 模拟宿主提供的绝对路径
  const absoluteDistPath = path.resolve(__dirname, '../../dist');
  console.log(`[Test] Using absolute path for loading: ${absoluteDistPath}`);
  it('应该能够通过绝对路径加载并运行', () => {
    // 1. 完整性检查
    const isValid = NativeLoader.checkIntegrity(absoluteDistPath);
    expect(isValid).toBe(true);

    // 2. 动态加载
    const ffmpeg = NativeLoader.load<IFFmpegModule>(absoluteDistPath);
    const instance = ffmpeg.MediaManager.getInstance();
    // 3. 调用功能
    const result = instance.getMediaInfo("C:\\Users\\gpy\\Desktop\\qqqa.mp4");
    console.log('Media Info:', result);
...
describe('FFmpeg Player Core Functional Tests', () => {
  ...
  const videoPath = "E:\\project\\deepcool\\nodejs\\module_deepcool\\module-ffmpeg-core\\test\\qqq.mp4"

  // --- 1. 全局初始化 ---
  beforeAll(() => {
    console.log(`[Setup] Testing with video: ${videoPath}`);
    manager = ffmpeg.MediaManager.getInstance();
  });
  ...
  test('Step 1: Get Media Info (Static)', () => {
    ...
    try {
        const info = manager.getMediaInfo(videoPath);
